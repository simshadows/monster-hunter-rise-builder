...
for (const [skillID, skillObj] of hardcodedSkills) {
    ...
    assert(isNonEmptyStr(skillObj.id));
    assert(isInt(skillObj.shortId) && (skillObj.shortId > 0));
    assert(isNonEmptyStr(skillObj.name));
    assert(isInt(skillObj.maxLevels));

    assert(strHasOnlyLowerUnder(skillObj.id));
    // Name can have any format.
    assert((skillObj.maxLevels > 0) && (skillObj.maxLevels < 8)); // Change if needed
    
    // And now, we check for duplicates and add

    assert(!skillMap.has(skillObj.id));
    assert(!skillMapShortIds.has(skillObj.shortId));
